...
from server import average_gradients,average_gradients_unbiaised,average_gradients_unbiaised_corr,apply_gradient,apply_gradient_clipped,monte_carlo_expectation_corr,real_expectation_corr
...
def run_experiment(M1, M2, e1, e2, N=500):
    ...
    dataloaders = get_dataloaders_group_iid_ap(32 , [0,1,2] , [3,4])#get_dataloaders_one_label_per_client(32,5)
    ...
    epsilons_active = {0 : 0.7,1 : 0.1,2  : 0.5,3 : 0.1,4 : 0.8}
    epsilons_nonactive = {0 : 0.9,1 : 0.8,2 : 0.9,3 : 0.8,4 : 0.9}
    """
    ...
    biases = []
    for m in clients: 
        bias = monte_carlo_expectation_corr(m, group1, group2, epsilons_access, epsilons_active, epsilons_nonactive, N=10000)
        biases.append(bias)
    ...
    for rnd in range(N):
        ...
        if M_t != 0:
            ...
            avg_grad = average_gradients_unbiaised_corr(grads_all,group1,group2, biases)#,client_lengths) 
...
def main(): 
    ...
    set_seed(42)
    biased_loss_rnds , unbiased_loss_rnds , unbiased_loss_th_rnds , rounds , rounds2 = run_experiment(M1=3, M2=2, e1=0.8, e2=0.2 , N=500) 
    ...
    plt.title("Comparison of Training Loss Over Rounds (e1 = 0.8 , e2 = 0.2) iid labels per ap with higher probabilities for client in AP2 lr = 1/sqrt(T)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    #plt.ylim(0, 4) 
