...
def run_experiment(M1, M2, M3, e1, e2 , e3 , N=500):
    ...
    #print(f"group1 : {group1} len(group1) : {len(group1)} , group2 : {group2} , len(group2) : {len(group2)} , group3 : {group3} , len(group3) : {len(group3)}")
    ...
    for rnd in range(N):
        ...
        if M_t != 0 :
            ...
            apply_gradient_normalized(global_biased, avg_grad, lr)
    ...
    for rnd in range(N):
        ...
        for cid in dataloaders:
            ...
            grads_all.append(grad)
        avg_grad = average_gradients(grads_all,M_t)#,client_lengths)
        apply_gradient_normalized(global_unbiased, avg_grad, lr)
        if (rnd + 1) % 100 == 0 : 
            print(f"\n--- Communication Round {rnd+1} ---")
            avg_grad_array = np.concatenate([v.ravel() for v in avg_grad.values()])
            norm = np.linalg.norm(avg_grad_array)
    ...
    for rnd in range(N):
        ...
        if M_t != 0:
            ...
            lr = 0.01 * mean_bias
            #print(f"avg_grad : {avg_grad}")
            #avg_grad = clip_gradient(avg_grad , max_norm)
            apply_gradient_normalized(global_debiased, avg_grad, lr)
            if (rnd + 1) % 100 == 0 : 
                print(f"\n--- Communication Round {rnd+1} ---")
...
def main(): 
    ...
    plot_with_std(rounds_shared, unbiased_mean, unbiased_std, "Unbiased", 'green', '--')

    plt.xlabel("Communication Round")
    plt.ylabel("Squared norm of the gradient of the loss function")
    plt.title("Gradient Norm Over Rounds (Averaged over 5 seeds) (Normalized updates)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
