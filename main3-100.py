...
from data_loader import get_dataloaders_iid,get_dataloaders_one_label_per_client,get_dataloaders_group_iid_ap,get_dataloaders_label_per_ap
from server import average_models,variance_gradients
from client import compute_stochastic_gradient,compute_online_bias
from server import average_gradients,average_gradients_unbiaised,average_gradients_unbiaised_corr,apply_gradient,apply_gradient_clipped,monte_carlo_expectation_corr,real_expectation_corr,average_gradients_unbiaised_3groups_corr,monte_carlo_expectation_3groups_corr
...
def run_experiment(M1, M2, M3, e1, e2 , e3 , N,seed):
    ...
    epsilons_active = {0 : 0.93,1 : 0.92 ,2  : 0.94,3 : 0.05,4 : 0.03, 5 : 0.05  , 6 : 0.03 , 7 : 0.92 , 8: 0.97 , 9 : 0.92}
    epsilons_nonactive = {0 : 0.9,1 : 0.8,2 : 0.9,3 : 0.8,4 : 0.9 , 5: 0.8 , 6 : 0.8 , 7 : 0.8 , 8 : 0.9 , 9 : 0.8}
    #epsilons_nonactive = {0 : 0.95,1 : 0.92,2 : 0.91,3 : 0.93,4 : 0.94 , 5: 0.98 , 6 : 0.89 , 7 : 0.95 , 8 : 0.93 , 9 : 0.94}
    """ 
    ...
    lr = 0.0447 #0.01 * mean_bias #0.0447 * mean_bias # 0.01 * mean_bias #0.0447 * mean_bias#0.01 * mean_bias
    avg_grads_debiased = []
    rounds_debiased = []
    X = [[] for _ in range(M)]
    ...
    for rnd in range(N):
        ...
        for cid in dataloaders : 
            if participations[cid] == 1 : 
                ...
                X[m].append(0)
            bias = compute_online_bias(X[m], delta )
            biases.append(bias)
        print(f"mean of biases : {np.mean(biases)}")
    ...
    return biased_loss_rnds , unbiased_loss_rnds , debiased_loss_rnds , avg_grads_biased , avg_grads_unbiased , avg_grads_debiased ,  rounds_biaised , rounds_debiased
...
def main(): 
    seeds = [42 , 123 , 7]
    biased_losses_total = []
    unbiased_losses_total = []
    debiased_losses_total = []

    avg_grads_biased_total = []
    avg_grads_unbiased_total = []
    ...
    for seed in seeds :
        set_seed(seed)
        biased_loss_rnds , unbiased_loss_rnds , debiased_loss_rnds , avg_grads_biased , avg_grads_unbiased , avg_grads_debiased , rounds , rounds2 = run_experiment(M1, M2 , M3 , e1, e2, e3 ,N,seed) 

        biased_losses = np.array([i[0] for i in biased_loss_rnds])
        unbiased_losses = np.array([i[0] for i in unbiased_loss_rnds])
        debiased_losses = np.array([i[0] for i in debiased_loss_rnds])

        biased_losses_total.append(biased_losses)
        unbiased_losses_total.append(unbiased_losses)
        debiased_losses_total.append(debiased_losses)

        #print(f"biased_losses_total : {biased_losses_total} , unbiased_losses_total : {unbiased_losses_total} , debiased_losses_total : {debiased_losses_total}")
        avg_grads_biased_total.append(avg_grads_biased)
        avg_grads_unbiased_total.append(avg_grads_unbiased)
        avg_grads_debiased_total.append(avg_grads_debiased)

        

    
    biased_losses_mean = np.mean(biased_losses_total,axis = 0)
    unbiased_losses_mean = np.mean(unbiased_losses_total,axis = 0 )
    debiased_losses_mean = np.mean(debiased_losses_total,axis = 0)

    avg_grads_biased_mean  = np.mean(avg_grads_biased_total,axis = 0)
    avg_grads_unbiased_mean  = np.mean(avg_grads_unbiased_total,axis = 0)
    avg_grads_debiased_mean  = np.mean(avg_grads_debiased_total,axis = 0)
    ...
    plt.title("Comparison of Training Loss Over Rounds (e1=0.92, e2=0.02 , e3 = 0.95) 1 label per client (differenet p) , M1 = 3, M2 = 4 , M3 = 3, lr = 0.01 ")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    #plt.ylim(0, 4) 
    ...
    #print(f"rounds : {rounds} , biased_losses_mean : {biased_losses_mean} , rounds2 : {debiased_losses_mean} , rounds : {rounds} , unbiased_losses_mean : {unbiased_losses_mean}")
    plt.plot(rounds, biased_losses_mean, label="Biased", linestyle='-', marker='o')
    plt.plot(rounds2, debiased_losses_mean , label="Debiaised", linestyle='-', marker='o',color = 'red')
    plt.plot(rounds, unbiased_losses_mean, label="Unbiased", linestyle='--', marker='s')

    plt.xlabel("Communication Round")
    plt.ylabel("Training Loss")
    plt.title("Comparison of Training Loss Over Rounds (Online estimation) (e1=0.92, e2=0.02 , e3 = 0.95)  1 label per client (AP2 higher p) , M1 = 3, M2 = 4 , M3 = 3, lr = 0.01 ")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    #plt.ylim(0, 4) 
    ...
    plt.plot(rounds, avg_grads_biased_mean, label="Biased", linestyle='-', marker='o')
    plt.plot(rounds2, avg_grads_debiased_mean , label="Debiaised", linestyle='-', marker='o',color = 'red')
    plt.plot(rounds, avg_grads_unbiased_mean , label="Unbiased", linestyle='--', marker='s')

    plt.xlabel("Communication Round")
    plt.ylabel("Squared norm of the gradient of the loss function")
    plt.title("Comparison of Training Loss Over Rounds (e1=0.92, e2=0.02 , e3 = 0.95) 1 label per client (AP2 higher p) , M1 = 3, M2 = 4 , M3 = 3, lr = 0.01 ")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    #plt.ylim(0, 4) 
